...
static int crashdump_init(struct msm_gpu *gpu, struct crashdump *crashdump)
{
	...
	crashdump->ptr = msm_gem_get_vaddr_locked(crashdump->bo);
	if (!crashdump->ptr)
		goto out;
...
out:
	if (ret) {
		if (crashdump->ptr) {
			msm_gem_put_vaddr_locked(crashdump->bo);
			crashdump->ptr = NULL;
		}
		...
		u32 reg, u32 val)
{
	...
	BUG_ON(crashdump->index + (2 * sizeof(u64)) >= CRASHDUMP_SCRIPT_SIZE);
		...
		u32 reg, u32 count, u32 offset)
{
	...
	BUG_ON(crashdump->index + (2 * sizeof(u64)) >= CRASHDUMP_SCRIPT_SIZE);
	BUG_ON(offset + (count * sizeof(u32)) >= CRASHDUMP_DATA_SIZE);
...
{
	BUG_ON(!crashdump->ptr || offset >= CRASHDUMP_DATA_SIZE);
		...
		u32 offset)
{
	...
	for (i = 0; i < count; i++) {
		SNAPSHOT_WRITE_U32(snapshot,
			CRASHDUMP_DATA_READ(crashdump, offset++));
	}
		...
		struct msm_snapshot *snapshot)
{
	...
	/* If we don't have a crashdumper dump what we can through AHB */
	if (!crashdump) {
	...
	for (i = 0; adreno_gpu->registers[i] != ~0; i += 2) {
		...
		CRASHDUMP_SCRIPT_READ(crashdump,
			adreno_gpu->registers[i], count, offset);
	...
	if (crashdump_run(gpu, crashdump)) {
		/* Crashdump failed, get what we can from the CPU */
		a5xx_snapshot_registers_cpu(gpu, snapshot);
		return;
	}
	...
	/* Copy the "regular" registers */
	for (offset = 0, i = 0; adreno_gpu->registers[i] != ~0; i += 2)
		offset += _copy_registers(snapshot, crashdump,
			adreno_gpu->registers[i],
			REG_COUNT(&(adreno_gpu->registers[i])), offset);
		...
		u32 size, u32 offset)
{
	...
	SNAPSHOT_MEMCPY(snapshot, CRASHDUMP_DATA_PTR(crashdump, offset),
		size * sizeof(u32));
}
